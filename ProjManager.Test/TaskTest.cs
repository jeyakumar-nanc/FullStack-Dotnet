...
namespace ProjManager.Test
{
    ...
    class TaskTest
    {
        ...
        public void AddParentTaskTest()
        {
            ...
            var response = taskSvc.AddParentTask(item);


            var result = taskSvc.GetAllParentTasks();
            Assert.IsInstanceOf(typeof(OkNegotiatedContentResult<List<PARENT_TASK>>), result);
            var expectedResponse = ((OkNegotiatedContentResult<List<PARENT_TASK>>)result).Content.Last();
            

            if (expectedResponse != null)
            {
                Assert.AreEqual(expectedResponse.ParentTask, item.TaskName);
            };
        }
        ...
        public void EndTaskTest()
        {
            ...
            var response = taskSvc.EndTask(item);
            var expectedRes = taskSvc.GetTaskbyId(19);
            Assert.IsInstanceOf(typeof(OkNegotiatedContentResult<TaskData>), expectedRes);
            TaskData actual = ((OkNegotiatedContentResult<TaskData>)expectedRes).Content;
        ...
        [Test]
        public void EndTaskByWrongIdTest()
        {
            var item = new TASK
            {
                TaskName = "Implementation - Tech",
